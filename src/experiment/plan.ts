...
export type StrokePlan = { plannerVersion: string; brushVersion: number; seed: number; size: 1024; analysisSize: number; detailSize?: 1024; qualityMetrics?: { candidates: number; rejected: number; acceptedGain: number; protectedDamage: number; scratchBytes: number; referenceBytes: number }; composition: Composition; inputHash: string; stages: { name: string; end: number }[]; strokes: PlannedStroke[]; materials?: Materials; processVersion?: 1; pickups?: PaintPickup[]; processMetrics?: { movedStrokes: number; previousTravel: number; travel: number; previousScore: number; score: number; pickups: number; costModel?: string; previousActionScore?: number; actionScore?: number; previousPickups?: number; previousBrushChanges?: number; brushChanges?: number } };
