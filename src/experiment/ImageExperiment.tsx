...
export function ImageExperiment({ back }: { back: () => void }) {
  ...
  function choose(file?: File) {
    if (!file) return;
    stopWork();
    if (runtime.current?.player?.index) setPending({ kind: 'replace', file });
  ...
  function changeComposition(next: Composition) {
    ...
    setMessage(next === 'crop' ? '居中方形将裁去预览框外内容。请检查主体；再次确认前不会改变右侧作品。' : '完整保留原比例，空白处铺上浅色笔触。再次确认前不会改变右侧作品。');
  }
