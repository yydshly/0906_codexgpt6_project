...
export function ImageExperiment({ back }: { back: () => void }) {
  ...
  const [approach, setApproach] = useState<PaintingApproach>('original');
  ...
  function changeApproach(next: PaintingApproach) {
  ...
  return <dialog ref={dialog} className="image-experiment" aria-label="图片自动绘制实验" onCancel={event => { event.preventDefault(); if (pending) setPending(null); else leave(); }} data-version={version}>
    ...
    <div className="painting-approach" role="group" aria-label="绘制方式"><div><span className="eyebrow">这一幅，怎样落笔</span><div className="approach-options"><button aria-pressed={approach === 'original'} disabled={exporting} onClick={() => changeApproach('original')}>原版 · 油画笔触</button><button aria-pressed={approach === 'structure'} disabled={exporting} onClick={() => changeApproach('structure')}>结构优先 · 实验</button><button aria-pressed={approach === 'quality'} disabled={exporting} onClick={() => changeApproach('quality')}>成品细节 · 实验</button></div></div><p>{approach === 'original' ? '保留现有画风，逐层铺色与细化。' : approach === 'quality' ? '尝试保留更多可见细节；本轮人物成品验证未通过，仍可能丢失结构。' : '优先局部轮廓，同一区域同笔同色连续画。仍保留油画笔触，人脸细节可能失真。'}<small>{player ? `当前画作：${planApproach(player.plan) === 'quality' ? '成品细节' : planApproach(player.plan) === 'structure' ? '结构优先' : '原版油画'}${preparationValid ? '' : ' · 重新准备前保留原作'}` : '选择方式后，再准备笔与颜色。'}</small></p></div>
