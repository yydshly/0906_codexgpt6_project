...
export function MaterialBoard({ materials, player }: { materials?: Materials; player?: PlanPlayer | null }) {
  const stroke = player?.plan.strokes[player.index];
  return <aside className="prepared-materials" aria-label="备色板与笔架">
    ...
    <section className="prepared-board" data-testid="paint-palette" data-action={player?.action ?? 'idle'} aria-label="固定色盘">
      <span className="prepared-engraving">SLOWLIGHT · OILS</span><span className="prepared-hole" aria-hidden="true"/>
      {materials ? <div className="prepared-dishes">{materials.dishes.map(d => <div key={d.id} className="prepared-dish" data-dish-id={d.id} data-color={d.color} data-active={stroke?.dishId === d.id} aria-label={`${d.id} ${d.color}`}><span className="prepared-well" style={{ backgroundColor: d.color }}/><span>{d.id}</span></div>)}</div> : <p className="materials-empty">先确认构图<br/>再为这幅图准备颜色</p>}
    </section>
    ...
    <div className="prepared-brushes" style={{ gridTemplateColumns: `repeat(${materials?.brushes.length || 1}, minmax(0, 1fr))` }}>{materials ? materials.brushes.map(b => <div key={b.id} className="prepared-brush" data-brush-id={b.id} data-away={player?.heldBrushId === b.id} aria-label={`${b.id} ${b.size} 像素${player?.heldBrushId === b.id ? '已取出' : '在笔架'}`}>
