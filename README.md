...
当前分支 `codex/m2-guided-creation` 实现 M2：保留同页平头笔绘画、覆盖/基础 RYB 混色和最近 20 笔撤销，增加旅行日落四步引导、一个本地草稿的保存/恢复、签名与完成预览。导出为实际画作和签名的原生 1024×1024 PNG。没有预制作品或运行时模型调用。
...
现有线上站点：[慢光数字油画室](https://yydshly.github.io/0906_codexgpt6_project/)，仍为此前部署的 M1。M2 仅在开发分支按步骤提交推送，不合并 main、不手动触发 Pages，也不修改部署工作流。
...
“签名与完成”打开实际作品预览，可返回修改或下载同一份 PNG。签名会印在右下角，参考层不导出。本地草稿按浏览器和站点地址隔离，不会同步到另一个浏览器或当前线上 M1；清理浏览器数据也可能丢失，请导出留存。
...
- 本轮仅原定 M2，不含账号、收费、社区、图层平台、云同步、图片转笔触或通用回放。不合并或部署。
