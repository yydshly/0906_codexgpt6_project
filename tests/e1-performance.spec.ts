...
test('continuous fixed complex scene without screenshot or encoding interference', async ({ page }) => {
  ...
  writeFileSync(`${root}/continuous.json`, JSON.stringify({ status: '通过', fixture: 'complex.jpg', composition: 'contain', speed: 1, recording: false, screenshots: false, exportDuringMeasurement: false, batches, planningFrames, playbackFrames, ...data, note: 'rAF intervals are scheduling observations, not input latency or human hand-feel.' }, null, 2));
});
