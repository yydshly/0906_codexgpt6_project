...
test('continuous fixed complex scene without screenshot or encoding interference', async ({ page }) => {
  ...
  if (process.env.E1_APPROACH === 'structure' || process.env.E1_APPROACH === 'quality') await page.getByRole('button', { name: process.env.E1_APPROACH === 'quality' ? '成品细节 · 实验' : '结构优先 · 实验', exact: true }).click();
  ...
  const data = await page.evaluate(() => { const w = window as any, e = window.__experiment!; w.__measuring = false; w.__observer.disconnect(); return { frames: w.__frames as { gap: number; phase: string }[], longTasks: w.__longTasks, wallMs: performance.now() - w.__start, planningMs: e.planningMs, strokes: e.plan!.strokes.length, metrics: e.player!.metrics, timing: e.player!.timing, memory: e.painting.memory(), qualityMetrics: e.plan!.qualityMetrics, renderer: e.renderer.info() }; });
