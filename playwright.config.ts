...
export default defineConfig({
  ...
  projects: [
    { name: 'e1-engine', testMatch: ['e1-engine.spec.ts', 'e1-materials-engine.spec.ts'] },
    { name: 'e1-performance', testMatch: 'e1-performance.spec.ts', timeout: 1500000, use: { video: 'off', screenshot: 'off' } },
    { name: 'e1', testMatch: ['e1.spec.ts', 'e1-evidence.spec.ts', 'e1-refinement.spec.ts', 'e1-brush-process.spec.ts', 'e1-prepared.spec.ts'], timeout: 1800000, use: { video: { mode: 'on', size: { width: 1440, height: 900 } }, screenshot: 'only-on-failure' } },
